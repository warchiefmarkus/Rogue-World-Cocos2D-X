...
copy_my_files(){
    cp $COCOS2DX_ROOT/template/android/run.sh $APP_DIR/android
    sed "s/__run_package_path__/$PACKAGE_PATH/;s/__run_project_name__/$APP_NAME/" $COCOS2DX_ROOT/template/android/run.sh > $APP_DIR/android/run.sh
    chmod u+x $APP_DIR/android/run.sh
    
    cp $COCOS2DX_ROOT/template/android/copy_resources.sh $APP_DIR/android
    sed "s/__cr_app_name__/$APP_NAME/;s#__cr_cocos2dx_root__#$COCOS2DX_ROOT#" $COCOS2DX_ROOT/template/android/copy_resources.sh > $APP_DIR/android/copy_resources.sh
    chmod u+x $APP_DIR/android/copy_resources.sh
    }
